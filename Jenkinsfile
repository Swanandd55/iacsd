	...
	stages {
		...
		stage('Deploy to K8s')
		{
			steps{
				...
				{
					sh 'scp -r -o StrictHostKeyChecking=no node-deployment.yaml ec2-user@3.139.238.163:/home/ec2-user/'
					
					script{
						try{
							sh 'ssh ec2-user@3.139.238.163:/home/ec2-user/ kubectl apply -f node-deployment.yaml --kubeconfig=/path/kube.yaml'
